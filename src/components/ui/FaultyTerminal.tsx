...
export default function FaultyTerminal({
  ...
  mouseReact = true,
  mouseStrength = 0.2,
  dpr = typeof window !== 'undefined' ? Math.min(window.devicePixelRatio || 1, 1.5) : 1, // Cap DPR at 1.5 for performance
  ...
  useEffect(() => {
    ...
    function resize() {
      if (isCleanedUp || !ctn || !renderer) return;
  ...
  }, [dpr]); // Only re-init on DPR change
  ...
  useEffect(() => {
    ...
    (program.uniforms.uTint.value as Color).set(tintVec[0], tintVec[1], tintVec[2]);
  ...
  // Animation loop - separate from setup, respects visibility
  useEffect(() => {
    const program = programRef.current;
    ...
    const update = (t: number) => {
      // Check cancellation before scheduling next frame
      if (isCancelled) return;
      rafRef.current = requestAnimationFrame(update);

      // Skip rendering when not visible (visibility pause)
      if (!isVisible) return;
  ...
  }, [isVisible, pause, timeScale, mouseReact, pageLoadAnimation]);
